  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* User Context Strip */}
        {!isCollapsed && (
          <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/60 flex items-center justify-between">
            <div className="truncate pr-2">
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold truncate mt-0.5">
                {user?.department || 'Student'} • {isFaculty ? user?.role : `Sem ${user?.semester || 6}`}
              </p>
            </div>
            {onToggleCollapse && (
              <button
                onClick={onToggleCollapse}
                className="p-1.5 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition cursor-pointer"
        ...
        {isCollapsed && onToggleCollapse && (
          <div className="flex justify-center">
            <button
              onClick={onToggleCollapse}
              className="p-2.5 rounded-2xl text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer"
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center rounded-2xl text-xs transition-all duration-200 cursor-pointer active:scale-[0.98] ${
                        ...
                        isActive
                          ? 'bg-brand-600 text-white font-extrabold shadow-md shadow-brand-500/25'
      ...
      {!isCollapsed && (
        <div className="px-3 py-3 rounded-2xl bg-slate-50/70 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/80 flex items-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-brand-500 flex-shrink-0" />
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold">
            Smart Campus v2.1 Pro
          </span>
        </div>
      )}
    </aside>
